...
public class NumberController {
    private static final String DB_URL = "jdbc:mysql://localhost:3306/yeblodb";
    ...
    private int fetchValueFromTable(int categoryCode) {
        try
             ...
             PreparedStatement statement = connection.prepareStatement("SELECT value FROM numbers WHERE CategoryCode = ? ")) {
    ...
    private void updateTable(int categoryCode, long nextNumber) {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
             PreparedStatement statement = connection.prepareStatement("UPDATE numbers SET value = ? WHERE CategoryCode = ?")) {
